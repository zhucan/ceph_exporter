...
RUN echo "deb https://download.ceph.com/debian-pacific focal main" >> /etc/apt/sources.list
...
WORKDIR $APPLOC
RUN go get -d
# The `-tags pacific` instructs go-ceph to enable additional support pacific release.
# See https://github.com/ceph/go-ceph#installation
RUN if [ -n "${TEST}" ]; then go test -tags pacific -v -race -count=1 ./...; fi
RUN go build -gcflags=all="-N -l" -tags pacific -o /bin/ceph_exporter
